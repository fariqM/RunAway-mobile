    ...
    for (var i = 0; i < 4; i++) {
      switch (i) {
        ...
        case 1:
          formattedData.push(
            this.props.metric
              ? (rowData[1] * 1.609).toFixed(2) + ' km'
              : rowData[1].toFixed(2) + ' mi',
          );
          break;
        case 2:
        ...
        case 3:
          formattedData.push(
            this.props.metric
              ? this.formatPace((rowData[3] * 0.621).toFixed(2)) + ' min/km'
              : this.formatPace(rowData[3]) + ' min/mi',
          );
          break;
      }
    }
    ...
    this.props.runs.forEach(run => {
      if (run.id === id) {
        this.setState({selectedRun: run}, () => {
          ...
          const modalData = [];
          modalData.push(
            'Time: ' + this.formatTime(this.state.selectedRun.time) + '\n',
          );
          modalData.push(
            'Distance: ' +
              (this.props.metric
                ? (this.state.selectedRun.distance * 1.609).toFixed(2) + ' km\n'
                : this.state.selectedRun.distance.toFixed(2) + ' miles\n'),
          );
          modalData.push(
            'Pace: ' +
              (this.props.metric
                ? this.formatPace(this.state.selectedRun.pace * 0.621) +
                  ' min/km\n'
                : this.formatPace(this.state.selectedRun.pace) + ' min/mi\n'),
          );
          modalData.push(
            'Calories: ' + this.state.selectedRun.calories.toFixed() + '\n',
          );
      ...
      <ScrollView>
        <View>
          ...
          <Text style={styles.totals}>
            {' '}
            Total Time: {this.formatTime(this.props.total_time)}{' '}
          </Text>
          <Text style={styles.totals}>
            {' '}
            Total Distance:{' '}
            {this.props.total_distance > 0
              ? this.props.metric
                ? (this.props.total_distance * 1.609).toFixed(2) + ' km'
                : this.props.total_distance.toFixed(2) + ' miles'
              : '0.00' + (this.props.metric ? ' km' : ' miles')}
          </Text>
          <Text style={styles.totals}>
            {' '}
            Average Pace:{' '}
              ...
              ? this.props.metric
                ? this.formatPace(
                    this.props.total_time /
                      60 /
                      (this.props.total_distance * 1.609),
                  ) + ' min/km'
                : this.formatPace(
                    this.props.total_time / 60 / this.props.total_distance,
                  ) + ' min/mi'
              : '0:00' + (this.props.metric ? ' min/km' : ' min/mi')}
          </Text>
          <Text style={styles.totals}>
            {' '}
            Total Calories:{' '}
            {this.props.total_calories
              ? this.props.total_calories.toFixed()
              : '0'}{' '}
          </Text>
            ...
            <TableWrapper style={styles.head}>
              {this.state.tableHead.map((title, index) => (
                <Cell
                  ...
                  onPress={() =>
                    this.sortTable(index, this.state.ascendingSort)
                  }
                />
              ))}
            </TableWrapper>
            ...
            <View style={{marginTop: 22}}>
              <View>
                ...
                <MapView
                  ...
                  <Polyline
                    coordinates={
                      this.state.route.length > 0 ? this.state.route : []
                    }
                    strokeColor="#000"
                    strokeColors={[
                      '#7F0000',
